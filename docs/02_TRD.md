...
│  of record      │ 11 tbl·59,985  │ │ import objects │ │ segment (see §9) │   │
│                 └────────────────┘ └────────────────┘ └──────────────────┘   │
│                                                                              │
...
| Relational store | **Data Store** | 11 tables, 59,985 FIRs, queried with ZCQL. |
...
1. **Entity resolution** — block on name key + district, then `rapidfuzz` similarity with
   **rarity-aware distinctiveness** (a rare surname match counts for far more than a common
   ...
   clusters. 54,337 accused rows → 52,928 identities (578 repeat), each with a confidence. **No protected
   attributes.**
...
7. **Health metrics** — deterministic: reporting delay, ageing vs peer median, pendency,
   undetected risk, false-case pattern. 26,168 cases flagged.
